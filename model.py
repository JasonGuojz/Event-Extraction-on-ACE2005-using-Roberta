    ...
    def predict_arguments(self, sen_mask_id, argument_keys, arguments_2d, adjm):
        ...
        for k, trigger in enumerate(argument_keys):
            ...
            argument_hidden.append(
                torch.cat([x[i], trigger_BERTemb, eventtype_emb], dim=-1))  # [SEQ_LEN,hidden_size*2+tri_embsize]
        ...
        for idx in event_index:
            ...
            argument_loss = argument_loss + self.arg_CRF[int(idx)].neg_log_likelihood_loss(feats=argument_logits,
                                                                                           mask=mask[event_index[idx]],
                                                                                           tags=arguments_y_2d[
                                                                                               event_index[idx]])
